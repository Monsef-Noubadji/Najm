...
function publish(): void {
  ...
  const versions = dirs.map((dir) => JSON.parse(readFileSync(`packages/${dir}/package.json`, 'utf8')).version as string);
  ...
  const args = tag === 'next' ? ['run', 'release', '--', '--tag', 'next'] : ['run', 'release'];
