...
for (const dir of ['najm', 'najm-compiler', 'najm-router', 'najm-server']) {
  ...
  assert.match(changelog, new RegExp(`^# .+\\r?\\n\\r?\\n## ${expected.replaceAll('.', '\\.')}`, 'm'), `${pkg.name} changelog must lead with the RC`);
}
...
console.log('release version: coordinated 1.0.0-rc.1 state verified');
